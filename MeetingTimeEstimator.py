...
class MeetingTimeEstimator:
    ...
    @staticmethod
    def estimate_meeting_times_unmet_random_walks(meeting_times, max_time_steps):
        """
        ...
        * Replace each -1 (i.e. each sample of the meeting time to be estimated) with: v + max_time_steps + 1, where v is a value sampled from the geometric distrbution with parameter p.
        Here we are using the memoryless property of the geometric distribution. 
        ...
        n_unment_random_walks = len(meeting_times) - np.count_nonzero(meeting_times+1)
        if n_unment_random_walks == 0:
            return meeting_times

        # Otherwise we estimate the meeting time as described above
        else:
            # Estiamte the parameter p of the geometric distibution 
            p = MeetingTimeEstimator.estimate_p_geometric_approximation(meeting_times=meeting_times, max_time_steps=max_time_steps)

            # Find the indices of the samples where the walks did not meet
            index_unmet_walks = np.argwhere(meeting_times==-1).reshape(-1)

            # Replace the value -1 at the indices in index_unmet_walks with the estimate constructed as decribed above
            meeting_times_with_estimate = np.zeros(len(meeting_times))
            meeting_times_with_estimate[index_unmet_walks] = np.random.geometric(p=p, size=len(index_unmet_walks)) + max_time_steps + 1
            meeting_times_with_estimate = meeting_times_with_estimate + meeting_times

            return meeting_times_with_estimate

    @staticmethod
    def estimate_p_geometric_approximation(meeting_times, max_time_steps):
        """
        ...
        n_unment_random_walks = len(meeting_times) - np.count_nonzero(meeting_times+1)
        probability_not_met = n_unment_random_walks/len(meeting_times)
