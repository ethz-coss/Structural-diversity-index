...
To install the package and its dependencies you should create a `python virtual environment <https://docs.python.org/3/library/venv.html>`_.
A detailed tutorial about virtual environments is available `here <https://docs.python.org/3/tutorial/venv.html>`_.
...
The reason is that pip cannot install the Cudatoolkit (because it is not a python package).
...
Now you can set the flag on_cuda to True (see Examples.ipynb in `GitHub <https://github.com/ethz-coss/Structural-diversity-index>`_) and computations will run on GPUs.
